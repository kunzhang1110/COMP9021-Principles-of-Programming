...
class PriorityQueue():      # max heap: from largest to smallest
    def __init__(self, capacity = MIN_CAPACITY, function = (lambda x, y: x > y)):
    ...
    def _bubble_up(self, i):  # i is self_length
        if i > 1 and self.function(self._data[i], self._data[i // 2]):
            self._data[i // 2], self._data[i] = self._data[i], self._data[i // 2]
            self._bubble_up(i // 2)

    def _bubble_down(self, i):
        child = 2 * i
        if child < self._length and self.function(self._data[child+1], self._data[i]):
            child += 1 # child is now the biggest child
        if child <= self._length:
            if self.function(self._data[child], self._data[i]): # and not equal
