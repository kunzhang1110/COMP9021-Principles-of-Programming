        ...
        for c in self.expression:
            if c.isdigit():
                ...
                if c in '+-*/(){}[]':
                    ...
                    return False
        if reading_number:  # last number
            list_of_tokens.append(number)
        return True

    def evaluate(self):
        for token in self.list_of_tokens:
            if token in list('+-*/([{'):  # if a integer
                ...
                try:
                    ...
                    other_parentheses = self.stack.pop()
                    if (other_parentheses == ')' and token != '(') or \
                        token == ']' and other_parentheses != '[' or \
                       token == '}' and other_parentheses != '{':
                        raise Exception
                except:
