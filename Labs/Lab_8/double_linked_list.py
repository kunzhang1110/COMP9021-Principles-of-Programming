...
class DoublyLinkedList:
    ...
    def reverse(self):
        ...
        node.next_node = None    # break all the links
        self.reverse()
    ...
    def insert_value_at(self, value, index):
        ...
        if next_node:       # in case the last node
    ...
    def insert_value_before(self, value_1, value_2):
        if not self.head:
            return False
        if self.head.value == value_2:  # prepend
            self.insert_value_at(value_1, 0)
            return True
        node = self.head
        while node.next_node and node.next_node.value != value_2: # find node with value_2
            node = node.next_node
        if not node.next_node:  # if there is node is the last node
            return False
    ...
    def insert_sorted_value(self, value, comparison_function = lambda x, y: x <= y):
        ...
        if not self.head:
            self.head = new_node
            return
        if comparison_function(value, self.head.value): # prepend
            new_node.next_node = self.head
            self.head.previous_node = new_node
            self.head = new_node
            return
