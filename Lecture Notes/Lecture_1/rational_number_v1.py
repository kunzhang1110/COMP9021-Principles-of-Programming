...
def get_input(input_name):
    while True:
        ...
        try:
            ...
            if number_input < 0:
                raise Exception
            return number_input, len(text_input)
        except:
            print('Incorrect input, try again.')

print('We want to compute a fraction, say a / b, that evaluates to .(sigma)(tau)(tau)(tau)...')

sigma, sigma_length = get_input('sigma')
tau, tau_length = get_input('tau')
...
numerator = sigma * (10 ** tau_length - 1) + tau
denominator = (10 ** tau_length - 1) * 10 ** sigma_length
if numerator == 0:
    denominator = 1
